...
quick_uni_analysis = function(variable, description) {
  hist(variable, xlab = description, 
     main = paste('Shapiro p-value:',
                  round(as.numeric(shapiro.test(variable)[2]), 8)
                  ))
  hist(log(crime$crmrte), xlab = paste('Log of ', description), 
     main = paste('Shapiro p-value:', 
                  round(as.numeric(shapiro.test(log(variable))[2]), 8)
                  ))
}
```
...
```{r, fig.height=3, fig.width=3, fig.show='hold'}
quick_uni_analysis(crime$polpc, 'Police as Percent of Pop')
```
...
```{r, fig.height=3, fig.width=3, fig.show='hold'}
quick_uni_analysis(crime$avgsen, 'Per. of Pop. That Are Young Males')
```
