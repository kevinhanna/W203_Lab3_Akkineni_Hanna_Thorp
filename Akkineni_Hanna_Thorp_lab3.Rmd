...
columns_pretty_sort = c("county", "year", "crmrte", "prbarr", "prbconv",
...
```{r}
crmrte.outliers = boxplot(crime$crmrte, plot = FALSE)$out
length(crmrte.outliers)
```
...
West shows a higher mean police per capita and a lower crime rate, which is likely more related to the

```{r}
region_model = lm(log(crmrte) ~ polpc, data = crime[crime$west, ])
```
...
Based on the table above, we We anticipate that the crime rate depends on certainty of punishment and severity of punishment. Also, we anticipate that polpc and density has an impact on the crime rate. As such, our base model includes variables that attempt to operationalize those concepts
...
summary(model1.b$coefficients)
```
From the coefficents summary, we can observe that the county 115 has a very high impact on our proposed model. This will vary our CLM assumptions.
...
cor_model1 <- data.matrix(subset(crime,
           select = c("log_crmrte","prbarr","prbconv","log_polpc","log_density")))
cor_model1 = cor(cor_model1)
diag(cor_model1) = 0 # Zero the diagonal as we are uninterested in those.
(cor_model1)
```
From the above matrix, we can see that crime rate is correlated with all the variables, and all other independent variables are very less correlated with each other except log_density and prbarr. Lets run the variable inflation factor and see if the `r cor(crime$log_density,crime$prbarr)` is a problem
...
Let's also check how these variables are jointly affecting crime rate. 
```{r}
model1.c = lm(log_crmrte ~ (prbarr) +  log_density, data = crime)
f <- summary(model1.c )$fstatistic
    (pf(f[1],f[2],f[3],lower.tail=F))
```
The p-value of the entire model indicates that both these variables are jointly significant.
...
```
There are some influential values however cook's distance is within the bounds. At value 51 and 79. if we remove those values and plot the graph, . 
...
```{r}
model1_intrepreation <-  c("","For approximately each 1 unit increase in probability of arrest,
                crime rate decreases by 1.92%","For approximately each 1 unit increase in probability of conviction,crime rate decreases by 0.69%", "For a 1% increase in polpc, crime rate increases by 55.5% when everything else stays the same","For an increase of one unit (100 people per square mile) in density,crime rate increases by 11.1% when everything else stays the same")
...
The adjusted $R^2$ for this model is 67.3% which means a lot of the the variations are explained by this model:
...
AIC(model1)
```
The Akaike information criterion (AIC) indicates that the relative quality of predicting crime rate based on our variables is 57.7 when only little information is lost.
...
model2 = lm(log_crmrte ~ (prbarr) + (prbconv)+ log_polpc + log_density + pctmin80 + 
...
####checking if county 71 that has both west=central=1 has any impact on our model. 
```{r}
model2.a = lm(log_crmrte ~ (prbarr) + (prbconv)+ log_polpc + log_density + pctmin80 + 
...
####checking if county 115 that has high prbarr and low crime and low polpc and very low density has any impact on our model. 
```{r}
model2.b = lm(log_crmrte ~ (prbarr) + (prbconv)+ log_polpc + log_density + pctmin80 + 
...
cor_model2 <- data.matrix(subset(crime,
           select = c("log_crmrte","prbarr","prbconv","log_polpc","log_density","pctmin80","west")))
cor(cor_model2)
```
From the above matrix, we can see that crime rate is correlated with all the variables, and all other independent variables are very less correlated with each other except west and pctmin80. Lets run the variable inflation factor and see if the `r cor(crime$west,crime$pctmin80)` is a problem
```{r}
vif(model2)
```
Based on pairwise correlation and variance inflation factors, we do not detect evidence of multicollinearity negatively impacting our specification.
...
```{r}
model2_intrepreation <-  c("","For approximately each 1 unit increase in probability of arrest,
                crime rate decreases by 1.82%","For approximately each 1 unit increase in probability of conviction,crime rate decreases by 0.69%", "For a 1% increase in polpc, crime rate increases by 73% when everything else stays the same","For an increase of one unit (100 people per square mile) in density,crime rate increases by 10.4% when everything else stays the same","For every 1 unit increase in percent minority, crime rate increases by 1%","For all western counties crime rate decreases by 1.81% compared to other counties","For all western counties, a 1% increase in polpc reduces the crime rate by 0.26%")
...
The adjusted $R^2$ for this model is 82.2% which means a lot of the the variations are explained by this model:
...
AIC(model2)
```
The Akaike information criterion (AIC) indicates that the relative quality of predicting crime rate based on our variables is 9.02 when only little information is lost.
