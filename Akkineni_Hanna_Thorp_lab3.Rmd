...
There is a small change in the coefficient of west, -1.81 - (-2.09) = 0.3 when county 71 is removed.  This change of ~0.3 represents bias in west if county 71 in fact in central NC, however county 71, Gaston County, is in Western NC and keeping it in our model when int including the central will provide the correct results.   
...
In addition to the variables included in model 2, we analyzed the effects of percentage of younge males and tax revenue per capita on crime rate. Next, based on the analysis of wages  above, we include wcon and wfed to our model due to thier stronger explainatory power as an example why they and other wages were not included in our optimal specification in model 2. We are not using any interaction terms here and want to check how crime rate is varying across all regions for these variables.
...
```
From the coefficents summary, we can observe that the county 115 doesn't have any big impact on our coefficients relative to the model when including county 115.
