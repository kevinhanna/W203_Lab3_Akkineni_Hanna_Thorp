...
columns_prety_sort = c("county", "year", "crmrte", "prbarr", "prbconv",
...
### Correlation Matrix
```{r}
corrmatrix <- cor(crime[,columns_prety_sort[3:26]])
corrplot(corrmatrix, cl.pos = "b", diag = FALSE)
```

Using the correlation matrtix above we can see there are some strong correlations between Crime Rate (crmrte) and Population Densisty (density), Crime Rate and Urban, East and Percentage Minority from 1980 (pctmin80) and West and Percentage Minority, interestingly opposit correlations in those last two. We also see stong corelation with Populations Dessity and the Average Wage (avgwage) and several wages, specifically trade, financial, insurance real estate and federal employees.
...
aggregate(polpc ~ region, data = crime, mean)
