...
Crime-Determinants: we anticipate that the crime rate depends on average sentencing as higher crime rate tends to have higher sentencing. And Increased avgsen suggests there are severe crimes happening in a given county.
...
Some toughts on wealth inequality.
1. A larger pay gap would mean more variance, thus more poverty for otherwise similar mean.
2. It is poverty that is related to crime, not the pay gap.
3. Poverty is partially captured by tax and wage as it will take the mean down in a similar way

I feel in many of the cases, we're actually looking for say, percentage of people below the poverty line, or at several levels.  Otherwise we're wishing to replace a proxy with a proxy. 
