...
Crime rate has `r length(crmrte.outliers)` outliers, though it is not enough to cause concern.
...
cor_model1 <- data.matrix(subset(crime,
                select = c("prbarr","prbconv","log_polpc","log_density")))
cor(cor_model1))
```

We can see from the previous correlation matrix, Crime Rate is highly correlated with the variables in our model. We also see that probability of arrest and density are highly correlated: `r cor(crime$prbarr,crime$density)`.
...
cor_model2 <- data.matrix(subset(crime,
                select = c("prbarr","prbconv","log_polpc","log_density","pctmin80","west")))
cor(cor_model2))
```
