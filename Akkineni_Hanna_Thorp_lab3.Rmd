---
title: 'Lab 3: Reducing Crime (FINAL: Stage 3)'
...
We were provided with a dataset that includes variables for 90 counties in North Carolina. This data appears to have been collated from:
    1)crime statistics from the North Carolina Department of Corrections' prison and probation files;
    2)demographic statistics taken from the decennial census;
    3)police data derived from FBI police agency data; and
    4)wage data from the North Carolina Employment Security commission.  
...
#### Create avgwage and east Variables
...
boxplot(log(crime$crmrte), main = 'log Crime Rate')
plot(log(crime$crmrte))
```
...
The log transformed crime rate has a single outlier, it is not significant enough to cause concern.
...
Both tax revenue per capita and the average wages are positively correlated with people per square mile.  This is relavent since those are positively correlated with crime rate, the opposite of what we would expect.  However, since they are correlated with people per square mile, which is, as expected, strongly possitively correlated with crime rate, this will likely account for some of the correlation in tax revenue per capita and average wages, along with some ommited variables discussed later in the analysis. 

```{r echo = FALSE, results = 'asis'}
kable(codebook[-c(1:3,15:23), c(2,3,5,7)], "latex", longtable = TRUE, booktabs = TRUE, caption = "Explanatory Variable Summary") %>%
  kable_styling(full_width = TRUE, latex_options = c("HOLD_position", "striped", "repeat_header"), row_label_position = 1)%>%column_spec(1,width="2em")%>%column_spec(2,width="5em")%>%column_spec(3,width="8em")%>%column_spec(4,width="5em")  
...
# Model Analysis
Based on the correlation table above, we are apply transformations to the following variables and storing them in our data frame.
```{r}
...
$$
\begin{aligned}
log(crmrte) &= \beta_0 + \beta_{1}prbarr + \beta_{2}prbconv + \\
...
<!-- I think since west and central are not included in the model it's a safe assumption that this quality issue will not affect the outcom. KH

#### Intuition of the effects of potential data issues in county 71 on the model
...
```
From the coefficents summary, we can observe that the county 115 has a very high impact on our proposed model. This county can impact our CLM assumptions, particularly for the variables operationalizing certainty of punishment.

### Testing the validity of the 6 CLM assumptions

#### CLM 1: Linear model 

The mode l is specified such that the dependent variable is a linear function of the explanatory variables.
...
kable(cor_model1_out, "latex", longtable = TRUE, booktabs = TRUE, caption = "Correlation Matrix: Model 1") %>%
  kable_styling(full_width = TRUE, latex_options = c("HOLD_position", "striped", "repeat_header"), row_label_position = 1)  
```

In the matrix above, we find that probability of arrest and density are highly correlated: `r cor(crime$prbarr,crime$log_density)`.
We also test the probability of arrest and density alone are jointly affecting crime rate. 
```{r}
...
Index 51 and 79 - 
Density and polpc are quite low for 51 and 79, while the crime rate and probability of arrest are similar to other counties. Of note, these are both western counties.
...
The adjusted $R^2$ for this model is `r summary(model1)$adj.r.squared`% which means a lot of the the variation in crime rate is explained by this model

The Akaike information criterion indicates that the relative quality of predicting crime rate based on our variables is `r AIC(model1)`.
...
`west`, `pctmin80`, and an interaction of `west` and `polpc`. This is because as seen in the below table, western counties have a higher concentration of police, but a lower crime rate on average. This is opposite to what we have observed in our #Model1. 

```{r  fig.height=4, fig.width=4.5, fig.align='center', fig.show='hold'}
boxplot(crime$crmrte[crime$east],crime$crmrte[crime$central],
crime$crmrte[crime$west], names=c("East","Central","West"),
...
```
From the coefficents summary, we can observe that the county 115 deosn't appear to have a very high impact on our proposed model. It is possible that adding a control for west and the interaction between west and polpc has mitigated this broader influence. 
...
kable(cor_model2_out, "latex", longtable = TRUE, booktabs = TRUE, caption = "Correlation Matrix: Model 2") %>%
  kable_styling(full_width = TRUE, latex_options = c("HOLD_position", "striped", "repeat_header"), row_label_position = 1, font_size = 8)  
```
...
From the residuals vs fitted plot, the residuals are largely centered on 0 except few values. 
**We have not uncovered evidence that would lead us to question the validity of assumption.**
...
Observations 51, 79 and 90 are influential.  
The density and polpc are very low, whereas the crime rate, probability of arrest, probability of conviction and the percent of minority are similar to other counties. These outliers are all in western counties.  
If we remove those values and plot the graph: 
...
Model conclusion - 
Probability of arrest has more impact on crime rate. One is more likely to be arrested than convicted, which is expected as an arrest is usually nessisary for a trial. From the arrests, we can see 50% are not convicted. Also, We observed that crime rate is low for western counties and high for eastern counties. This confirms our intuition.

The adjusted $R^2$ for this model is `r summary(model2)$adj.r.squared`% which means a lot of the variation of the dependent variable is explained by this model.

The Akaike information criterion (AIC) indicates that the relative quality of predicting crime rate based on our variables is `r AIC(model2)`.

## Model 3 - Sub-optimal Specification

In addition to the variables included in model 2, we analyzed the effects of percentage of young males and tax revenue per capita on crime rate. Next, based on the analysis of wages above, we include wcon and wfed in our model due to their strong explainatory power as an example why they and other wages were not included in our optimal specification in model 2. We are not using any interaction terms here and want to check how crime rate varies across all regions for these variables.
...
```
From the above two coefficents summaries, we can observe that the county 115 doesn't have any big impact on our coefficients.
...
kable(cor_model3_out, "latex", longtable = TRUE, booktabs = TRUE, caption = "Correlation Matrix: Model 3") %>%
  kable_styling(full_width = TRUE, latex_options = c("HOLD_position", "striped", "repeat_header"), row_label_position = 1)  
```


We do see strong correleation between wfed and wcon `r cor(crime$wcon,crime$wfed)` along with polpc and density. 
We examine how these variables affect crime rate: 
```{r}
...
The p-value of the entire model $`r pf(f[1],f[2],f[3],lower.tail=F)`$ indicates that these variables are jointly significant. But the coefficients of wcon and wfed are close to zero. 
```{r}
vif(model3)
```
...
plot(model3, which=5)
```
There are some influential values - values 59 and 79. 
...
Index 59 - 
This is an eastern county with low crime rate and low polpc, differing from many other observations in our model. We do not think that this observation merits exclusion. 
...
polpc, density and pctmin80 when we look at heteroskedastic-robust errors. But taxpc, pctymle, wcon and wfed don't have much signifance. But all these variables are jointly significant:
...
The Akaike information criterion (AIC) indicates that the relative quality of predicting crime rate based on our variables is `r AIC(model3)` .

The adjusted $R^2$ for this model is `r summary(model3)$adj.r.squared`% which means a lot of the the variation in the dependent variable is explained by this model. That said, we have hit the point of diminishing returns from adding more variables as these values are quite close to those for the more parsimoneous model 2. 
...
&\beta_{11}percent\_of\_population\_previously\_imprisoned + ... + error
\end{aligned}
$$

We believe it should be relatively strightforward to add in variables operationalizing education, poverty rate, real wage, proportion of population that is young, the low skill unemployment rate, the proportion of the population that previously committed a crime, and the portion that was previously imprisoned. Unfortunately, we are unable to observe the rest of our desired concepts.  

Some concepts have been operationalized in our dataset. For example, certainty of punishment has been operationalized through three variables: 1) the percent of the population which are police, 2) the ratio of arrests to crimes, and 3) the ratio of convictions to arrest. This is among the most effective operationlizations in this dataset. Severity of punishment is also operationalized  through 1) the proportion of convictions that result in a prison sentence and 2) the average length of a prison sentence. Nominal wages are operationalized in the dataset with average wages for certain industry groupings. None of poverty rate, education, social cohesion, weapon availability, cost of living, or the low skill unemployment rate are operationalized within this dataset.  
